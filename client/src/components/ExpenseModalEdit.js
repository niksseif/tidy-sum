...
class ExpenseModal extends Component {
    ...
    render() {
        const { expenseCategory,expenseAmount,expenseDescription,expenseTotal} = this.state
       
        return (
            <div style={{ marginTop: '5vw', marginBottom: '5vw', marginLeft: '3vw', marginRight: '3vw' }}>
                <Form>
                    <Form.Group>
                        <Form.Field control={Input} icon="dollar sign" placeholder={expenseCategory} label='EXPENSE CATEGORY' onChange={this.handleChange} name='category' />
                        <Form.Field control={Input} placeholder={expenseAmount} label='EXPENSE AMOUNT' onChange={this.handleChange} name='amount' />
                        <Form.Field control={Input} placeholder={expenseTotal} label='EXPENSE TOTAL' onChange={this.handleChange} name='total' />
                    </Form.Group>
                    <Form.Field
                        control={TextArea}
                        icon="dollar sign"
...
export default connect(mapStateToProps, { editUserExpense })(ExpenseModal)
